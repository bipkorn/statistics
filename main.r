...
#  TIME SERIES
  ...
  ggplot(daily_sales, aes(x=date, y=sales)) +geom_line(color="blue") +labs(title="Daily Total Sales",x="Date",y="Total Sales ($)") +theme_minimal()
  
  ### Ts Age
  #### Plot daily average age
  ggplot(daily_sales, aes(x=date, y=avg_age)) + geom_line(color="blue") + labs(title="Daily Average Age of Customers", x="Date",y="Average Age") +theme_minimal()
...
# 1.3 Scatter view
  ...
  ## invoice date
  ggplot(data, aes(x=invoice_date, y=quantity)) +geom_point(alpha=0.5, color="blue") +labs(title="Scatter Plot: Quantity vs. invoice_date",x="invoice_date",y="Quantity") +theme_minimal()
  
  ## Sales data
  ggplot(data, aes(x=sales, y=quantity)) + geom_point(alpha=0.5, color="blue") + labs(title="Scatter Plot: Sales vs. quantity", x="Sales", y="Quantity") +  theme_minimal()
  
  ## age
  ggplot(data, aes(x=age, y=quantity)) + geom_point(alpha=0.5, color="blue") + labs(title="Scatter Plot: Quantity vs. Age", x="Age", y="Quantity") + theme_minimal()
...
#Task 2
  ...
  m_model1= as.matrix(model1)
  m_model2= as.matrix(model2)
  m_model3= as.matrix(model3)
  m_model4= as.matrix(model4)
  m_model5= as.matrix(model5)
...
# task 3 
  ...
  for (i in 1:num) {
    ...
    New_Y_Hat <-  m_model5 %*% New_thetahat ## calculating new Y-hat
